# Phase 2.5 — Analyst Estimates Implementation Contract

Status: ARCHITECTURE APPROVED; implementation begins after checkpoint 2.5.0.
This is the authoritative Phase 2.5 contract, superseding provisional estimates
formulas in the educational blueprint (in particular its EPS floor and breadth
denominator). Frozen Phase 2.1, 2.2 and 2.4 calculations remain unchanged.

## Scope and invariants

Build a deterministic, point-in-time analyst EPS/revenue estimates layer.
Every observation, alias, fiscal mapping, reporting event, correction, withdrawal,
availability assertion and coverage assertion must satisfy `available_at <= T`.
No AI, cross-sectional rankings, factors, detector signals, portfolios,
backtesting or calibration. Do not begin Phase 2.6.

The preceding architecture review is preserved separately as historical context
in `phase-2.5-final-architecture-revision.md`; its opening verification statements
refer to the earlier read-only review, not the current implementation session.
...
Implement the canonical provider interface, deterministic normalization contract,
and synthetic provider fixtures. No commercial purchase or live provider is
required. Real connector qualification is separate and must not block the core.
...
## Implementation checkpoints and durable recovery

| Checkpoint | Deliverable and required gate |
| --- | --- |
| 2.5.0 | Freeze this contract; record actual baseline; diff check; commit/push. |
| 2.5.1 | Canonical models, constraints/indexes, Alembic migration; construction, uniqueness, NUMERIC zero, microsecond TIMESTAMPTZ, checks/FKs; existing and fresh PostgreSQL upgrade and safe downgrade/re-upgrade. |
| 2.5.2 | Provider interface, normalizer, dedupe/conflicts, timing, immutable observations, supersession validation, raw collision protection and replay hash verification. |
| 2.5.3 | Target, contributor, actual reporting and forward horizon resolvers; PIT aliases/mappings/reporting, January year-end, rollover, amendments, no compression. |
| 2.5.4 | Event replay, consensus calculation, atomic state writer and as-of repository; episodes, corrections/target moves, withdrawal, ordering, no old-head resurrection, relational provenance/idempotency. |
| 2.5.5 | Revision formulas, calendar lookbacks, same-target comparison, breadth/counts and completeness; every numeric example tested. |
| 2.5.6 | Exact 15 features and explicit runner allowlist; pinned calculation identity, snapshot persistence, provenance, ageing, rollover, legacy isolation. |
| 2.5.7 | Full PostgreSQL synthetic semiconductor integration: 3–5 contributors, Q1/Q2 EPS/revenue, initiation/revisions/withdrawal/correction, earnings boundary, T1/T2/T3, run-twice reproducibility. |
| 2.5.8 | Spec/implementation audit and all acceptance tests; fresh PostgreSQL proof; IMPLEMENTATION COMPLETE / READY FOR INDEPENDENT REVIEW / NOT YET FROZEN. |

After each logical checkpoint: run focused tests and regressions, `git diff
--check`, update PROJECT_STATE with files/models/migrations, decisions, exact test
counts, limitations and next checkpoint; commit, push origin/master and verify
HEAD equals origin/master with a clean working tree. Record the resulting commit
in a subsequent durable handoff update (a file cannot embed its own Git hash).
Do not claim completion before pushing. Do not accumulate checkpoints uncommitted.
...
cat docs/phase-2.5-estimates-spec.md
git log --oneline -15
git status --short
```

Fix failed gates before advancing; do not weaken assertions. Document and stop
for a material architecture contradiction. Missing provider credentials or
semantics do not block synthetic/core work. Preserve private configuration,
loopback-only PostgreSQL and existing SEC/clean-build fixes. Never commit secrets.
